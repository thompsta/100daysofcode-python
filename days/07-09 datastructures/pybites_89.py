...
def get_every_nth_state(states=states, n=10):
    ...
    # can use slice and step value (second :) to return every step/slice interval
    return states[n-1::n]
...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
    ...
    try:
        out = us_state_abbrev[state_name]
    except:
        out = NOT_FOUND
    return out
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    states.sort()
    return sorted(list(us_state_abbrev.values())[0:10]) + states[-10:]    
    

print(get_every_nth_state(states))

print(get_state_abbrev('Oregon'))
